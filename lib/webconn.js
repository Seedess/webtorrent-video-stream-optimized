...
class WebConn extends Wire {
  ...
  httpRequest (pieceIndex, offset, length, cb) {
    ...
    const getCacheData = (url, start, end) => {
      const cached = cache.find(entry => {
        if (entry.url === url && entry.start <= start && entry.end >= end) {
          return true
        }
      })
      if (cached) {
        return {
          ...
          res: cached.res,
          data: cached.data.slice(start - cached.start, end - cached.start + 1),
        }
      }
    }
    ...
    requests.forEach(request => {
      ...
      const fileEnd = request.fileEnd
      const maxEnd = Math.min(fileEnd, start + ((end-start) * 10))

      const cached = getCacheData(url, start, end)
      if (cached) {
        debug('using cached request range', cached)
        return onResponse(cached.res, cached.data, true)
      }
      ...
      const opts = {
        ...
        headers: {
          ...
          range: `bytes=${start}-${maxEnd}`
        },
        timeout: SOCKET_TIMEOUT
      }
      function onResponse (res, maxData, isFromCache = false) {

        const data = maxData.slice(0, end - start + 1)
